...
char* ns_convert(char* out, const char * number, unsigned int sourceBase, unsigned int destBase);
...
int main()
{
    ...
    for (i = 0; i < lenCases; i++) {
		...
		fflush(stdout);
        res = ns_convert(sourceNumber, tCase->sourceNumber, tCase->sourceBase, tCase->destBase);
